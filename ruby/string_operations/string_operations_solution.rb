...
# You're given a DICTIONARY constant.
DICTIONARY = Hash[('a'..'z').zip('A'..'Z')]
class String
  # build a "my_upcase" method below!
  def my_upcase
    upcased = chars.map do |char|
      DICTIONARY[char]
    end
    upcased.join
  end
  ...
  def my_capitalize
    "#{DICTIONARY[self[0]]}#{self[1..-1]}"
  end

  # build a "my_downcase" method below!
  def my_downcase
    downcased = chars.map do |char|
      DICTIONARY.key(char)
    end
    downcased.join
  end
  ...
  def my_gsub(old, replacement)
    gsubbed = chars.map do |char|
      char == old ? replacement : char
    end
    gsubbed.join
  end
  ...
  # build a "my_upcase!" method below!
  def my_upcase!
    # self = chars.map { |c| DICTIONARY[char] }.join
  end
end
